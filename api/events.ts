...
import { API_ENDPOINT, HEADER_APPLICATION_JSON_MAP, HEADER_TEXT_PLAIN_MAP, HEADERS_CROSS_ORIGIN_MAP } from "../src/services/api/contants.js";
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  if (!["GET", "POST", "OPTIONS"].includes(req.method as string)) {
    res.status(405).setHeaders(HEADERS_CROSS_ORIGIN_MAP).setHeaders(HEADER_TEXT_PLAIN_MAP);

    return res.send("Method Not Allowed");
  }

  if (![API_ENDPOINT, `${API_ENDPOINT}/`].includes(req.url as string)) {
    res.status(404).setHeaders(HEADERS_CROSS_ORIGIN_MAP).setHeaders(HEADER_TEXT_PLAIN_MAP);
  ...
  switch (req.method) {
    default:
    case "OPTIONS": {
      res.status(200).setHeaders(HEADERS_CROSS_ORIGIN_MAP).setHeaders(HEADER_TEXT_PLAIN_MAP);
    ...
    case "GET": {
      if (process.env.ANALOG_TOKEN && token !== process.env.ANALOG_TOKEN) {
        res.status(401).setHeaders(HEADERS_CROSS_ORIGIN_MAP).setHeaders(HEADER_TEXT_PLAIN_MAP);
      ...
      try {
        ...
        res.status(500).setHeaders(HEADERS_CROSS_ORIGIN_MAP).setHeaders(HEADER_TEXT_PLAIN_MAP);
