...
const ANALOG = {
  ...
  setStatus: function (newStatus: RequestStatus, error?: RequestError) {
    if (!canTransitionTo(this.state.status, newStatus)) {
      return;
    }
  ...
  renderErrorState: function (error: RequestError) {
    ...
    errorEl.innerHTML = `
      ...
      <div class="error-actions">
        ${config.showRetry ? '<button class="retry-button" id="retry-button">重新加载</button>' : ""}
      </div>
    `;
  ...
  renderData: async function () {
    ...
    try {
      ...
      const parsedError = parseFetchError(
        error.response,
        error as Error,
      );
  ...
  startTokenPolling: function () {
    if (this.state._isPollingActive) {
      return;
    }

    this.state._isPollingActive = true;
  ...
  stopTokenPolling: function () {
    this.state._isPollingActive = false;
  },
