...
export const mockData = (props?: {
  ...
  const envRange = Number(process.env.VITE_ANALOG_TIME_RANGE as string);
  const dateRange =
    props?.range || isNaN(envRange) ? MOCK_DATE_RANGE : envRange;
  ...
  Array.from(Array(recordsCount)).forEach(() => {
    dataset[Math.random().toString(36)] = Array.from(Array(dateRange))
      .map((_, i) => {
        ...
        return Array.from(Array(Math.floor(Math.random() * tsCount))).map(() =>
          date.getTime(),
        );
      })
      .flat();
